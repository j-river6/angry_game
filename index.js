...
function preload() {
  titleScreen = loadImage('angry_pumpkins.png');
...
function setup() {
  ...
  pumpkin = Bodies.circle(150, height - 162, 20, {
    isStatic: true
  });
  World.add(engine.world, pumpkin);

  slingshot = new SlingShot(150, height - 162, pumpkin);
  ...
  let link = createA('https://e0749igednw.feishu.cn/docx/FJ0XdEfcwouRejxta1bcsNu0nHf', '关注公众号，发现更多AI神器和有趣玩法');
  link.position(width / 2 + 40, 30); // 定位到画布下方
  link.style('color', '#fff'); // 设置链接颜色
...
class Particle {
  constructor(x, y) {
    ...
    this.lifespan = 255;
    this.size = random(10, 20); // tama帽o variable
  }
